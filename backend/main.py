...
async def update_transaction_status(
    ...
    security_manager.security_logger.info(
        f"Transaction action: {current_user['username']} -> {action.action} on {action.transaction_id}"
    )
...
async def generate_report(current_user: dict = Depends(oauth_handler.get_current_user)):
    ...
    try:
        ...
        security_manager.security_logger.info(
            f"Report generated by: {current_user['username']}"
        )
...
async def update_manit_status(
    ...
    security_manager.security_logger.info(
        f"MANIT status update: {current_user['username']} -> {transaction.status} on {transaction.transaction_id}"
    )
